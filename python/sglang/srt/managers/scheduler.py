...
class Scheduler(
    ...
    def run_batch(
        ...
        """Run a batch."""
        max_len = 0
        if batch.seq_lens_cpu is not None and len(batch.seq_lens_cpu) > 0:
            max_len = batch.seq_lens_cpu.max().item()

        with torch.profiler.record_function(f"Scheduler.run_batch max_len={max_len}"):
            self.forward_ct += 1
